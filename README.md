...
Raccoon 是基于商汤 [SenseNova](https://www.sensenova.cn/) 日日新大模型 开发的代码助手。

## 登录账号

您可以点击状态栏中的 `⧉` 图标，或在 Raccoon 助手 侧边栏视图可见时，点击面板标题栏上的 `齿轮图标` 进入设置页面进行登录。
...
* 侧边栏编程助手：点击 VS Code 侧边栏的 Raccoon 图标激活
* 编辑器内助手：在 Raccoon 侧边栏助手视图时，点击标题栏上的 `新窗口图标` 创建，可以创建多个独立实例，便于多线开发
* 终端面板内编程助手：在 Raccoon 侧边栏助手视图时，点击标题栏上的 `更多...` 选择 `新建 Raccoon 终端` 创建，同样的可以创建多个独立实例
...
* 在编辑器内单击右键选择 `向 Raccoon 提问...`，**侧边栏编程助手**
...
为了提供更加连续的开发体验，对于侧边栏助手页面，及工作区中未关闭的助手页面，Raccoon 会将历史会话信息本地缓存，这些信息会随着您的问答，操作（删除、清空）等同步更新内容。

当您想要主动清理保存会话历史信息的缓存文件时，可以在设置页面中点击 `清理历史缓存` 按钮。

## 使用 Notebook 创建可保存、可复现的对话文件 (Experimental)

您可以在 Raccoon 侧边栏助手视图时，点击标题栏上的 `更多...` 选择 `新建 Raccoon Notebook` 新建一个 Notebook 文件。
