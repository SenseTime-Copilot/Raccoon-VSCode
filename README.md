...
SenseCode 是基于商汤 [SenseNova](https://www.sensenova.cn/) 日日新大模型 开发的代码助手。

## 登录账号

您可以点击状态栏中的 `⧉` 图标，或在 SenseCode 助手 侧边栏视图可见时，点击面板标题栏上的 `齿轮图标` 进入设置页面进行登录。
...
编程助手通过连续对话方式为您解决问题，我们提供了三种方式来使用编程助手：

* 侧边栏编程助手：点击 VS Code 侧边栏的 SenseCode 图标激活
* 编辑器内助手：在 SenseCode 侧边栏助手视图时，点击标题栏上的 `新窗口图标` 创建，可以创建多个独立实例，便于多线开发
* 终端面板内编程助手：在 SenseCode 侧边栏助手视图时，点击标题栏上的 `更多...` 选择 `新建 SenseCode 终端` 创建，同样的可以创建多个独立实例

### 侧边栏及编辑器内的编程助手

#### 快速召唤编程助手

* 随时通过连按两次 `Ctrl` 键，召唤**侧边栏编程助手**
* 在编辑器内单击右键选择 `向 SenseCode 提问...`，**侧边栏编程助手**
* 在编辑器内选择部分代码，通过补全热键（默认为 `Alt + /`）触发 Code Action 菜单，其中将显示可处理代码的 SenseCode 命令，您可以直接选择执行<sup>*</sup>

> \* 当有任意编辑器内助手页**可见**时，Code Action 提问请求将优先在该**编辑器内助手**页回答。
...
您可以自定义提示词，以更好地适用于您的使用场景。在配置页面点击自定义提示词 `灯泡图标`，将帮您定位到 SenseCode 的设置中，您可以在此添加自己的提示词，插件将在侧边栏中生成对应的操作按钮。
...
在 SenseCode 侧边栏助手视图时，点击标题栏上的 `更多...` 选择 `管理代码收藏`，或通过设置页面中的管理代码收藏 `书签图标`，可以打开代码收藏管理页面，在这里可以添加、修改、删除代码片段。
...
相比于侧边栏或编辑器内助手，终端助手交互能力有限，无法删除前序对话来精细地修改上下文，也无法渲染输出结果及代码段，但如果需要在当前终端中新启会话，可以按 `Esc` 键，后续对话将不再参考之前的内容。
...
如当前工作空间存在一个或多个 Git 仓库，在 `源代码管理` 侧边栏面板中，会看到 `魔棒图标` 按钮，SenseCode 会理解您已暂存的更改，并在消息框中撰写合适的 Commit Message 建议。
