...
> 您也可以在 Raccoon 代码助手 的设置界面中，指定期望的界面显示语言，这仅影响 Raccoon 代码助手插件的内部显示语言。
...
* 快捷键：
  * 您可以点击配置段落标题右侧的 `键盘图标` 进入设置来进行修改
* 触发延时：
  * 配置从停止输入到触发获取补全建议请求的延时，也可配置为手动，此时仅当按下热键时获取补全建议，默认的热键为 `Alt + /`（mac: `⌥/`）
* 补全偏好：
...
您也可以部分接受结果，使用 `Ctrl + →` 接受单词建议，或使用 `Ctrl + ↓` 接受单行建议。
...
在设置页面中，可以配置 GUI 代码助手的

* 快捷键：
  * 您可以点击配置段落标题右侧的 `键盘图标` 进入设置来进行修改
* 结果显示：
  * 配置回答结果以流式实时返回，或等待全部结果生成后一次性返回
