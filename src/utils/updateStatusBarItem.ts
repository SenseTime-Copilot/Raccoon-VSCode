...
export async function updateStatusBarItem(
  ...
  let tmode = `\`${l10n.t("Hotkey")}\` [${l10n.t("Switch to automatic")}](command:sensecode.inlineSuggest.toggleAuto "Switch to automatic")`;
  let cmode = `\`${l10n.t("Snippets")}\` [${l10n.t("Switch to print")}](command:sensecode.inlineSuggest.togglePrintOut "Switch to print")`;
  statusBarItem.show();
  if (statusbartimer) {
  ...
  if (autoComplete) {
    tmode = `\`${l10n.t("Auto")}\` [${l10n.t("Switch to manual")}](command:sensecode.inlineSuggest.toggleAuto "Switch to manual")`;
  }

  let printOut = Configuration.printOut;
  if (printOut) {
    cmode = `\`${l10n.t("Print")}\` [${l10n.t("Switch to snippets")}](command:sensecode.inlineSuggest.togglePrintOut "Switch to snippets")`;
  }

  let activeEngine = context.globalState.get<Engine>("engine");

  let engine = `\`${l10n.t("None")}\` [${l10n.t("Goto settings")}](command:sensecode.settings "Set engine config")`;
  if (activeEngine) {
    if (Configuration.engines.length > 1) {
      engine = `\`${activeEngine.label}\` [${l10n.t("Change engine")}](command:sensecode.config.selectEngine "Change engine")`;
    } else {
      engine = `\`${activeEngine.label}\` [${l10n.t("Goto settings")}](command:sensecode.settings "Set engine config")`;
    }
  }
...
function buildTip(engine: string, tmode: string, cmode: string, err?: string) {
  ...
  tip.appendMarkdown(`$(gear) ${l10n.t("All Settings")}: [${l10n.t("Open setting page")}](command:sensecode.settings "Open setting page")\n\n`);
  if (err) {
    tip.appendMarkdown(`***\n\n`);
